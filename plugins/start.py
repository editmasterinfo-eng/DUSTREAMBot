...
async def get_stream_url(client, message_id, file_name_override=None):
    try:
        ...
        if msg.empty: return None, None
        
        file_name = file_name_override if file_name_override else get_file_name_robust(msg)
        clean_name = file_name.replace("_", " ")
        
        # 🔥 FIX: Ab direct clean url banega bina kisi filename ya extra slashes ke! (No 404 Risk)
        base_url = STREAM_URL.rstrip('/')
        direct_link = f"{base_url}/watch/{message_id}"
...
async def start(client, message):
    if not await checkdb.is_user_exist(message.from_user.id):
        ...
        else: return await message.reply("Wrong Input. /start again.")
        
        link = await client.ask(message.chat.id, "<b>Send Telegram Channel Link:\nEx :- <code>https://t.me/VJ_Bots</code></b>")
        if link.text and link.text.startswith(('http://', 'https://')): await db.set_link(message.from_user.id, link=link.text)
        else: return await message.reply("Wrong Input. /start again.")
        ...
        if message.from_user.id in ADMINS: buttons.append([InlineKeyboardButton("🔥 Open Firebase Panel", callback_data="fb_cat_list")])
...
async def universal_handler(client, message):
    ...
    try: log_msg = await message.copy(LOG_CHANNEL)
    except Exception as e: return await message.reply("Error saving file. Check bot permissions in Log Channel.")

    direct_link, clean_name = await get_stream_url(client, log_msg.id)
    if not direct_link: return await message.reply("Error generating link.")
    ...
    if message.from_user.id in ADMINS: buttons.append([InlineKeyboardButton("🔥 Add to Firebase", callback_data="fb_cat_list")])
...
async def link_start(client, message):
    ...
    try:
        ...
        else: await message.reply("Content not found.")
...
async def show_withdraw(client, message):
    clicks = get_count(message.from_user.id) or 0
    if clicks < 1000: return await message.reply("Minimum withdrawal is 1000 Plays.")
    if get_withdraw(message.from_user.id): return await message.reply("Withdrawal already pending.")
